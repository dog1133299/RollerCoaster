	...
	: QMainWindow(parent)
{
	...
	connect(ui.comboTrain, SIGNAL(currentIndexChanged(QString)), this, SLOT(ChangeTrainType(QString)));
	...
	connect( ui.rcpyadd		,SIGNAL(clicked()),this,SLOT(RotateControlPointAddY())				);
	connect( ui.rcpysub		,SIGNAL(clicked()),this,SLOT(RotateControlPointSubY())				);
	
	connect( ui.rcpBoxadd   ,SIGNAL(clicked()), this, SLOT(AddBox())							);
	connect( ui.rcpBoxsub   ,SIGNAL(clicked()), this, SLOT(SubBox())							);

	connect( ui.cave		, SIGNAL(clicked()), this, SLOT(addcave())							);
	connect(ui.people, SIGNAL(clicked()), this, SLOT(people()));
	connect(ui.w, SIGNAL(clicked()), this, SLOT(w()));
	connect(ui.YS, SIGNAL(clicked()), this, SLOT(Yspeaking()));
	connect(ui.GS, SIGNAL(clicked()), this, SLOT(Gspeaking()));
...
void AppMain::SwitchPlayAndPause()
{
	...
	{
		ui.bPlay->setIcon(QIcon(":/Roller Coaster/Resources/Icons/play.ico"));
		this->trainview->isrun = !this->trainview->isrun;
	}
	else
	{
		ui.bPlay->setIcon(QIcon(":/Roller Coaster/Resources/Icons/pause.ico"));
		this->trainview->isrun = !this->trainview->isrun;
	}
	if(this->trainview->isrun){
...
{
	if (val != 0) {
	this->trainview->speed = 1500 / val;
	this->trainview->tempspeed = 1500 / val;
	std::cout << val << "    ";
	}
...
void AppMain::AddBox()
{
	if (this->trainview->box <= 10)
		this->trainview->box++;
}

void AppMain::SubBox()
{
	if (this->trainview->box >1)
		this->trainview->box--;
}

void AppMain::close()
{
	this->trainview->close = !this->trainview->close;
}

void AppMain::addcave()
{
	this->trainview->cave = !this->trainview->cave;
}

void AppMain::w()
{
	this->trainview->wheel = !this->trainview->wheel;

}

void AppMain::people()
{
	this->trainview->people++;
	this->trainview->people=(this->trainview->people)%3;


}

void AppMain::Gspeaking()
{
	this->trainview->GS = !this->trainview->GS;
}

void AppMain::Yspeaking()
{
	this->trainview->YS = !this->trainview->YS;
}
...
void AppMain::rolly(float dir)
{
	...
	if (s >= 0) {
		...
		this->m_Track.points[s].pos.y+=dir*5;
...
void AppMain::RotateControlPointAddY()
{
	rolly(1);
}

void AppMain::RotateControlPointSubY()
{
	rolly(-1);
} 
...
void AppMain::ChangeTrainType(QString type)
{

	if (type == "train")
	{
		this->trainview->trainShape = 0;
	}
	else if (type == "box")
	{
		this->trainview->trainShape = 1;
	}
	else if (type == "bug")
	{
		this->trainview->trainShape = 2;
	}
	else if (type == "other")
	{
		this->trainview->trainShape = 3;
	}
...
//========================================================================
{
	//trainview->loadTexture2D("ct_BK.png", trainview->grass_ID);
